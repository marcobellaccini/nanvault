...
describe Nanvault do
  ...
  describe Nanvault::Encrypted do
    ...
    body_ok_salt = Bytes[52, 57, 52, 101, 56, 98, 50, 56, 49, 49, 56, 98, 55, 98, 101, 50, 48, 98, 54,
                    57, 102, 54, 57, 97, 53, 57, 54, 100, 50, 56, 52, 56, 56, 56, 49, 48, 53,
                    102, 99, 49, 101, 100, 53, 54, 98, 49, 54, 50, 97, 97, 51, 54, 98, 55, 57,
                    56, 48, 57, 48, 54, 48, 56, 51, 50]

    body_ok_hmac = Bytes[53, 102, 49, 54, 55, 57, 57, 55, 55,
                    49, 102, 100, 49, 55, 101, 99, 51, 99, 98, 55, 102, 97, 48, 51, 101, 56,
                    101, 102, 54, 51, 97, 48, 102, 48, 50, 54, 57, 50, 97, 97, 49, 49, 102, 97,
                    53, 100, 54, 53, 97, 48, 97, 51, 100, 54, 98, 100, 102, 102, 97, 101, 56, 51,
                    101, 100]
    
    body_ok_ctext = Bytes[100, 99, 101, 101, 97, 100, 99, 51, 98, 52, 102, 48, 57, 99,
                    53, 57, 99, 101, 49, 50, 49, 48, 50, 56, 100, 48, 57, 48, 57, 55, 100, 57,
                    55, 53, 102, 57, 51, 102, 99, 48, 54, 100, 102, 57, 51, 102, 55, 97, 100, 48,
                    101, 49, 49, 50, 56, 57, 51, 52, 99, 50, 102, 100, 97, 101]
  ...
  describe Nanvault::Crypto do
    ...
    salt = Bytes[52, 57, 52, 101, 56, 98, 50, 56, 49,
    ...
    exp_cipher_key = Bytes[78, 116, 215, 252, 194, 36, 178, 70, 82, 251, 119, 224, 218,
                      116, 83, 153, 69, 169, 197, 227, 207, 51, 20, 39, 194, 230,
                      183, 145, 74, 110, 205, 39]
    
    exp_hmac_key = Bytes[238, 131, 245, 97, 171, 142, 161, 134, 156, 85, 239, 81, 162,
                    17, 4, 52, 214, 103, 81, 58, 45, 156, 186, 98, 140, 179, 91,
                    35, 115, 63, 43, 66]

    exp_cipher_iv = Bytes[57, 38, 0, 68, 121, 201, 134, 53, 117, 66, 217, 173, 180, 21, 117, 76]

    ctext = Bytes[100, 99, 101, 101, 97, 100, 99, 51, 98,
                      52, 102, 48, 57, 99, 53, 57, 99, 101,
                      49, 50, 49, 48, 50, 56, 100, 48, 57,
                      48, 57, 55, 100, 57, 55, 53, 102, 57,
                      51, 102, 99, 48, 54, 100, 102, 57, 51,
                      102, 55, 97, 100, 48, 101, 49, 49, 50,
                      56, 57, 51, 52, 99, 50, 102, 100, 97, 101]

    describe "#get_keys_iv" do
      it "correctly get keys and iv" do
        cipher_key, hmac_key, cipher_iv = Nanvault::Crypto.get_keys_iv(salt, passbytes)
    ...
    describe "#decrypt" do
      it "correctly decrypt data" do
        cipher_key, hmac_key, cipher_iv = Nanvault::Crypto.get_keys_iv(salt, passbytes)
        ptext = Nanvault::Crypto.decrypt(cipher_iv, cipher_key, ctext)

        puts ptext.map { |x| x.as(UInt8).unsafe_chr }
      end
    end
